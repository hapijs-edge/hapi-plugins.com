...
var SearchApp = React.createClass({
    getInitialState: function () {
        this.performFetch = _.throttle(this.fetchFromServer, 240, {trailing: true});
        return {
            query: null,
            results: {}
        }
    ...
    fetchFromServer: function (query) {
        ...
        $.ajax({
            ...
            success: function (data) {
                SearchTreeCache[query] = JSON.parse(data);
                this.setState({
                    query: query
                });
            }.bind(this),
...
var SearchResults = React.createClass({
    render: function () {
        if (this.props.query && this.props.query.length > 0) {
            return (
                <ResultsTable query={this.props.query} results={this.props.results} searchTrees={this.props.searchTrees} />
            );
        } else {
            return (
                <SearchInfo />
...
var ResultsTable = React.createClass({
    render: function () {
        ...
        var tableRows = SearchTreeCache[this.props.query].map(function(plugin){
